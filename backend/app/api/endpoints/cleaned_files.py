"""
File management endpoints for the AI Assistant API.
This is the cleaned, production-ready version with improved file upload handling.
"""
import os
import shutil
import json
...
async def upload_file(
    ...
    name: Optional[str] = Form(None),  # Frontend sends name, we map to filename
    description: Optional[str] = Form(None),
    project_id: Optional[str] = Form(None),
    tags: Optional[str] = Form(None)  # Accept tags as string, will parse as JSON
) -> Any:
    """
    Upload a new file.
    """
    try:
        ...
        try:
            ...
            from ...db.models.document import Document as DocumentModel
            ...
            document = DocumentModel(
                ...
                tags=parsed_tags,  # Use parsed tags
                is_processed=False,
            ...
            # Return a dictionary response instead of relying on Pydantic validation
            return {
                "id": document.id,
...
@router.get("/status", response_model=ProcessingStats)  # Alternative endpoint
